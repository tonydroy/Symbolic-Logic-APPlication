...
public class MapABExpRootLayout extends AnchorPane {
    ...
    public MapABExpRootLayout(MapABExpView mapView) {
                ...
                for (Node node : nodesList) {
                    if (inHierarchy(event.getPickResult().getIntersectedNode(), node)) {
                        if (node instanceof MapABExpTreeFormulaBox) {
                            ((MapABExpTreeFormulaBox) node).processBoxRequest(event.getButton() == MouseButton.PRIMARY);
                            break;
                        }
                    }
                }
        ...
        starClickFilter = new EventHandler<MouseEvent>() {
            ...
            public void handle(MouseEvent event) {
                ...
                for (Node node : nodesList) {
                    if (inHierarchy(event.getPickResult().getIntersectedNode(), node)) {
                        if (node instanceof MapABExpTreeFormulaBox) {
                            ((MapABExpTreeFormulaBox) node).processStarRequest(event.getButton() == MouseButton.PRIMARY);
                            break;
                        }
                    }
                }
 //               event.consume();
        ...
        annotationClickFilter = new EventHandler<MouseEvent>() {
            ...
            public void handle(MouseEvent event) {
                ...
                for (Node node : nodesList) {
                    if (inHierarchy(event.getPickResult().getIntersectedNode(), node)) {
                        if (node instanceof MapABExpTreeFormulaBox) {
                            ((MapABExpTreeFormulaBox) node).processAnnotationRequest(event.getButton() == MouseButton.PRIMARY);
                            break;
                        }
                    }
                }
  //              event.consume();
        ...
        annotationPlus.setOnAction(e -> {
            ...
            for (Node node : nodesList) {
                if (node instanceof MapABExpTreeFormulaBox) {
                    ((MapABExpTreeFormulaBox) node).processAnnotationRequest(true);
                }
            }
        ...
        annotationMinus.setOnAction(e -> {
           ...
           for (Node node : nodesList) {
               if (node instanceof MapABExpTreeFormulaBox) {
                   ((MapABExpTreeFormulaBox) node).processAnnotationRequest(false);
               }
           }
            annotationToggle.setSelected(false);
        });
        ...
        circleClickFilter = new EventHandler<MouseEvent>() {
            ...
            public void handle(MouseEvent event) {
                ...
                for (Node node : nodesList) {
                    if (inHierarchy(event.getPickResult().getIntersectedNode(), node)) {
                        if (node instanceof MapABExpTreeFormulaBox) {
                            ((MapABExpTreeFormulaBox) node).processCircleRequest(event.getButton() == MouseButton.PRIMARY);
                            break;
                        }
                    }
                }
 //               event.consume();
        ...
        circleToggle.selectedProperty().addListener((ob, ov, nv) -> {
            if (nv)  main_pane.addEventFilter(MouseEvent.MOUSE_PRESSED, circleClickFilter );
                ...
                for (Node node : nodesList) {
                    if (node instanceof MapABExpTreeFormulaBox) {
                        ((MapABExpTreeFormulaBox) node).undoCircleRequest();
                    }
                }
            }
        });
        ...
        underlineClickFilter = new EventHandler<MouseEvent>() {
            ...
            public void handle(MouseEvent event) {
                ...
                for (Node node : nodesList) {
                    if (inHierarchy(event.getPickResult().getIntersectedNode(), node)) {
                        if (node instanceof MapABExpTreeFormulaBox) {
                            ((MapABExpTreeFormulaBox) node).processUnderlineRequest(event.getButton() == MouseButton.PRIMARY);
                            break;
                        }
                    }
                }
//               event.consume();
        ...
        underlineToggle.selectedProperty().addListener((ob, ov, nv) -> {
            if (nv)  main_pane.addEventFilter(MouseEvent.MOUSE_PRESSED, underlineClickFilter );
                ...
                for (Node node : nodesList) {
                    if (node instanceof MapABExpTreeFormulaBox) {
                        ((MapABExpTreeFormulaBox) node).undoUnderlineRequest();
                    }
                }
            }
        });
...
//                mDragOverIcon.startDragAndDrop (TransferMode.ANY).setContent(content);
    ...
    private void buildDragHandlers() {
                ...
                if (container != null) {
                    if (container.getValue("scene_coords") != null) {
                        ...
                        if (container.getValue("type").equals(DragIconType.dashed_line.toString())) {
                            slapp.editor.map_abexplain.MapABExpDashedLine line = new MapABExpDashedLine(mapView);
     ...
     //                  NodeLink link = new NodeLink();
                        slapp.editor.map_abexplain.MapABExpClickableNodeLink link = new MapABExpClickableNodeLink(mapView);
                        ...
                        for (Node n: main_pane.getChildren()) {
                            ...
                            if (n.getId().equals(sourceId))
                                source = (MapABExpTreeFormulaBox) n;

                            if (n.getId().equals(targetId))
                                target = (MapABExpTreeFormulaBox) n;
