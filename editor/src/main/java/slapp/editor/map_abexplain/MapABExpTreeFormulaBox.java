...
public class MapABExpTreeFormulaBox extends AnchorPane {
...
//    private ClickableNodeLink mDragLink = null;
    ...
    private final MapABExpTreeFormulaBox self;
    ...
    public MapABExpTreeFormulaBox(MapABExpView mapView) {
...
//        self.setBottomAnchor(mainBox, 0.0);
 ...
 //               startDragAndDrop (TransferMode.ANY).setContent(content);
    ...
    private BoxedDRTA newFormulaBoxedDRTA() {
        ...
        drta.getKeyboardSelector().valueProperty().setValue(mapView.getDefaultKeyboard());
...
//        rta.setContentAreaWidth(500);
 ...
 //       rta.getActionFactory().saveNow().execute(new ActionEvent());   ---------- this blanks text on open exercise.
        rta.focusedProperty().addListener((ob, ov, nv) -> {
            if (nv) {
                mapView.getMainView().editorInFocus(drta, ControlType.FIELD);
            }
        });
        return boxedDRTA;
    }
    ...
    private BoxedDRTA newAnnotationBoxedDRTA() {
        ...
        annRTA.focusedProperty().addListener((ob, ov, nv) -> {
            if (nv)  mapView.getMainView().editorInFocus(annBDRTA.getDRTA(), ControlType.FIELD);
            else {
